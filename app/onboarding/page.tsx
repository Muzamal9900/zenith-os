...
export default function OnboardingPage() {
  ...
  const fetchOnboardingState = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Error fetching onboarding state:', errorData)
        addToast({ type: "error", title: errorData.error || "Failed to fetch onboarding state" })
      }
    } catch (error) {
      console.log('Exception fetching onboarding state:', error)
  ...
  const updateOnboardingStep = async (stepId: OnboardingStepId, data: any) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Ensure the state is properly formatted before setting it
        const formattedState = {
          ...updatedState,
          createdAt: new Date(updatedState.createdAt),
          updatedAt: new Date(updatedState.updatedAt)
        }
        
        setOnboardingState(formattedState)
        ...
        if (updatedState.isCompleted) {
          addToast({ 
            type: "success", 
            title: "🎉 Onboarding Complete! Your White Label OS is ready to use!" 
          })
  ...
  const handleConfigurationNext = async (data: ConfigurationData) => {
    console.log('Configuration data received:', data)
    await updateOnboardingStep("configuration", data)
  }

  const handleToolSelectionNext = async (data: ToolSelectionData) => {
    await updateOnboardingStep("tool-selection", data)
  }

  const handleBillingPlanNext = async (data: BillingPlanData) => {
    await updateOnboardingStep("billing-plan", data)
  }

  const getCurrentStepComponent = () => {
    console.log('getCurrentStepComponent called, onboardingState:', onboardingState)
    ...
    console.log('currentStep:', currentStep, 'currentStepIndex:', onboardingState.currentStep)
